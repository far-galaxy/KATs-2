...
class NRF24:
    ...
    def begin(self, major, minor, ce_pin, irq_pin):
        ...
        self.spidev.max_speed_hz = 500000  # Maximum supported by NRF24L01+
    ...
    def setChannel(self, channel):
        if channel < 0 or channel > self.MAX_CHANNEL:
            raise RuntimeError("Chdannel number out of range")
    ...
    def write(self, buf):
        ...
        #print("monotonic() - send_at ", monotonic() - sent_at)
        #print("packet time ", packet_time)
        #print("timeout ", timeout, monotonic())
        
        while True: #monotonic() < timeout:
            time.sleep(packet_time)
            status = self.get_status()
            #print("status ", status, status & NRF24.TX_DS)
    ...
    def reset(self):
        """ Make sure the NRF is in the same state as after power up
            ...
            from other programs."""
        self.ce(0)
        reset_values = {0: 0x08, 1: 0x3F, 2: 0x03, 3: 0x03, 4: 0x03, 5: 0x02, 6: 0x0F,
